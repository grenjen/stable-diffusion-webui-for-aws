...
# download the SD model v1.5 and move it to the SD model directory
sudo -u ubuntu git clone --depth 1 https://huggingface.co/runwayml/stable-diffusion-v1-5
cd stable-diffusion-v1-5
sudo -u ubuntu git lfs pull --include "v1-5-pruned-emaonly.safetensors"
sudo -u ubuntu git lfs install --force
cd ..
mv stable-diffusion-v1-5/v1-5-pruned-emaonly.safetensors stable-diffusion-webui/models/Stable-diffusion/
rm -rf stable-diffusion-v1-5/
